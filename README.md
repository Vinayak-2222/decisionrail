...
`amount_at_risk` is the sole rupee multiplier in the EV calculation.
...
> The figures below are the exact numbers shown in the accompanying demo video's Baseline Comparison screen.
...
> **Note on reproducibility:** the baseline figures are exactly deterministic across runs (same seeded hidden outcomes: ₹146,614 recovered, 30.50% recovery rate — this reproduced exactly in every run we tested). The DecisionRail-side figures can vary slightly run-to-run because `train:estimator` does not currently fix a random seed for the logistic-regression training split/initialization. As a check, we independently re-ran the full pipeline from a completely fresh `git clone` on 2026-09-04 and got ₹167,458 recovered / 38.00% recovery rate / +7.50pp improvement — a different exact figure, but the same direction and similar magnitude of improvement over the fixed baseline. This confirms the *result* (DecisionRail meaningfully outperforms the baseline) is robust, even though the *exact rupee figure* depends on which estimator-training run produced it. Fixing the estimator's random seed is a natural follow-up improvement so future runs match bit-for-bit.
...
Both builds pass — confirmed on a clean `git clone` (2026-09-04) with no local dependency carryover.

### Full-stack reproducibility verification

The full stack (MongoDB, Redis, backend, frontend) was brought up via `docker-compose up --build` from a fresh clone with no pre-existing containers, volumes, or `.env` inherited from the development machine. All four containers started healthy, the backend connected to MongoDB and Redis, and the frontend served correctly on port 5173 with working login for both demo roles.
...
*(This section reflects an earlier pipeline run and has not yet been re-verified in the same fresh-clone session as the Current Measured Results above; re-run `npm run generate` → pipeline before final submission if you want this section on the same data snapshot.)*
...
Copy the example env file and fill in your own values before running the backend:
...
This full stack has been independently verified on a clean-machine / clean-clone Docker run (fresh `git clone` → `.env` created from `.env.example` → `docker-compose up --build`), confirmed on 2026-09-04.
...
- Secrets are kept in environment configuration (`.env`, gitignored) and out of source control; a placeholder `.env.example` is committed instead.
...
### Estimator training is not fully seeded

Unlike the synthetic data generation (which is fully seeded and deterministic), the logistic-regression training step (`npm run train:estimator`) does not currently pin a random seed. Re-running training can therefore produce a slightly different model and slightly different DecisionRail-side results on re-evaluation, even though the baseline and the underlying evaluation batch remain exactly fixed. The headline DecisionRail numbers in this README reflect one specific verified run; re-running the full pipeline may reproduce numbers in a similar range but not bit-for-bit identical figures. Fixing the estimator's random seed is a natural follow-up improvement.
...
3. **Seeded deterministic outcomes** for the synthetic world generation.
...
The project has a working frontend, backend pipeline, explainability workflow, Human Approval Queue, baseline comparison, audit trail, failure-injection harness, and synthetic experiment. The full stack, including Docker Compose, has been verified end-to-end from a clean `git clone` on 2026-09-04.

The current benchmark result is:

> **DecisionRail recovered ₹156,478 vs ₹146,614 for the fixed baseline on the identical synthetic evaluation batch, a ₹9,864 incremental recovery and +8.50 percentage-point recovery-rate improvement.** (An independent fresh-clone re-run on 2026-09-04 produced ₹167,458 / +7.50pp — see note above.)
